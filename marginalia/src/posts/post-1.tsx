...
export const post1: Post = {
  ...
  editorial: () => (
    <>
      ...
      <p>
        Not a "wow, look what AI can do" moment. A sad-truth moment.
      </p>
      <p>
      ...
      <p>
        I've felt that wall before. Years ago I had a passion project that
        required collecting and sorting through data. I actually got it
        done — but it quickly became a relic, because it relied on new
        ...
        when the formatting on a new piece was slightly different. I
        shipped. It became a relic.
      </p>
      <p>
        ...
        household size, and family composition. Every single number cited,
        kept current, re-checked when the source moves.
      </p>
      <p>
      ...
      <p>
        ...
        multi-day effort across all of them. Past-me, trying to be honest
        about what AI saved, was lowballing it.
      </p>
      <p>
  ...
  fieldNotes: () => (
    <>
      ...
      <p>
        ...
        PR reviews. Something to fix in the time log.
      </p>
      <p>
  ...
  raw: () => (
    <>
      ...
      <p>
        <strong>
          2026-05-07 — Setting up the AI time log is itself the
          highest-leverage entry [meta]
        </strong>
        <br />
        Spent ~6 minutes scaffolding <code>AI_TIME_LOG.md</code>; would have
        spent ~45 minutes solo (designing the table, arguing with myself
        about columns, second-guessing the multiplier framing). The act of
        tracking AI savings is itself an AI-saved task. Recursion is
        healthy.
      </p>
      <p>
        <strong>
          2026-05-07 — Force-pushing my own feature branch is still a mistake
          [process]
        </strong>
        <br />
        Asked Claude to "get my branch up to date with main." It rebased and
        force-pushed (with-lease). My instinct: that's not how I want to
        work — squash-merge at PR close means the feature-branch history
        doesn't matter, so a merge commit is fine and a
        rewrite-and-force-push is friction with no upside. Saved as a
        memory; default going forward is <code>git merge main</code>.
      </p>
      <p>
        <strong>
        ...
        <br />
        Accidentally deleted my Claude worktree and panicked about losing
        memory. Memory turned out to be in <code>~/.claude/</code> (separate
        from the worktree), so nothing was lost — but the panic itself
        surfaced that I'd been operating without a clear mental model of
        what's where. Worth knowing: memory is durable, worktrees are
        disposable, branches are recoverable via reflog. Don't overload one
        mechanism with assumptions about another.
      </p>
      <p>
        <strong>
        ...
        <br />
        End-of-session observation: "compact, compact, compact, or don't and
        hit your limit with 3 hours to sit around and do nothing." The
        auto-compact rhythm is the difference between continuing to work and
        burning quota waiting. No clean answer, just naming it — when
        sessions go long, compaction strategy starts mattering more than
        tool choice or model choice.
      </p>
      <p>
        <strong>
          2026-05-07 — Memory toggle was off and I didn't know [memory]
          [meta]
        </strong>
        <br />
        Realized late in the day: the in-product Claude Code memory feature
        wasn't toggled on. Unclear what that's affected. The file-based
        memory system Claude has been writing to (
        <code>~/.claude/projects/.../memory/</code>) is separate and gets
        loaded into context as system reminders at session start, so today's
        saves should be effective on next restart — but worth verifying
        explicitly on session resume rather than assuming. The "I'll save
        this to memory" moments don't always map to a single mechanism.
      </p>
      <p>
        <strong>
        ...
        <br />
        "One second I'm working in one directory, the next I'm making
        changes to the wrong version of the repo and wondering why nothing
        is changing for me on the app." The cwd-vs-main split is too easy
        to lose track of mid-task. Saved as a memory already (don't use
        worktrees unless asked). Open question for later: is there a
        project shape where worktrees actually pay rent here, or is
        single-checkout the permanent right answer?
      </p>
      <p>
        <strong>
          2026-05-07 — Letting AI prune branches felt good and scary [trust]
          [scope]
        </strong>
        <br />
        Deleted ~25 stale local + remote branches in one session, including
        20 force-pushed <code>--delete</code>s. "Felt so nice but kinda
        scary. Did it haphazardly with this project because I knew most of
        my stuff was safe and linear." Caveat for future: in larger
        projects, with collaborators, or when juggling multiple in-flight
        things, the same blast-radius action would warrant a more curated
        pass — verify each branch's status, ask before bulk deletes.
        Project context determines whether "drop the bomb" is OK or a
        mistake.
      </p>
      <p>
        <strong>
          2026-05-07 — Spot-correct beats fill-in-blank for backfill
          [process]
        </strong>
        <br />
        For the 22-PR time log backfill, Claude proposed estimates with
        confidence flags rather than asking 22 questions. Useful,
        "especially as memory fades." Convention going forward: when the
        user has to recall something fuzzy, lead with proposed numbers +
        uncertainty markers, not blank prompts. Recognition beats recall.
      </p>
      <p>
        <strong>
          2026-05-07 — The multiplier may be skewed low because solo would
          also produce worse output [trust]
        </strong>
        <br />
        Asked whether the calibrated ~4× multiplier matched my gut. Answer:
        lower than expected, but maybe not — solo would have meant a lot of
        research and learning-while-doing, which is a huge time sink and
        produces a less cohesive product (because the half-learning gets
        baked into the codebase as inconsistencies). The time log captures
        hours saved but can't capture the quality dimension: AI's leverage
        isn't just "Brian goes faster," it's "Brian doesn't have to
        half-learn five things at once and ship the half-learning as
        architecture."
      </p>
      <p>
        <strong>
          2026-05-07 — Some tasks are small enough to not use AI [scope]
          [process]
        </strong>
        <br />
        For small changes I could do in a few seconds of manual editing, I
        sometimes spent more time writing the prompt + waiting 2 minutes
        for processing than the change itself would have taken. The output
        was the exact single-line edit I'd already mentally drafted. AI
        has a fixed overhead per round-trip (prompt drafting + model
        latency + reading the response); below some change-size threshold,
        that overhead exceeds the change. Heuristic: if I can already see
        the diff in my head and it's &lt;5 lines, just type it.
      </p>
      <p>
        <strong>
          2026-05-07 — Figma is where AI made the impossible possible
          [skill] [tooling]
        </strong>
        <br />
        I am not a professional designer. I know basics, but I'm slow, I
        make mistakes, and I don't organize well because I don't live in
        Figma every day. Claude made the impossible possible here. The
        Figma file — Atlas Tokens, text styles, component variants, the
        swag explorations, the brand mark and lockups — would not exist in
        this shape without AI. The plugin API is gnarly (variable axes
        can't be set, skew decomposes to scale+rotation, text sizing has
        to be set on both axes), but Claude knows the patterns and the
        iteration loop is fast: screenshot → eval → tweak. This is the
        single biggest "would not have existed at all solo" line item in
        the project, bigger even than the audit pipeline.
      </p>
      <p>
        <strong>
          2026-05-08 — Take notes after every day of work, don't wait until
          morning
        </strong>
        <br />
        Lesson learned. My memory is fuzzy now. Maybe I'm just tired
      ...
      <p>
        <strong>
          2026-05-08 — Data import + sifting is where multi-day work
          collapses to hours [skill] [time-log]
        </strong>
        <br />
        This would have taken an enormous amount of time on my own. Getting
        this information imported. Sorting through it. Checking it. The
        3-hour solo estimates I logged on these PRs are a pipe dream — this
        was easily a multi-day effort across all of these PRs and files. AI
        is incredible at sifting through, sorting, and organizing data
        intelligently, which would normally take a lot more time by hand.
      </p>
      <p>
        <strong>
        ...
        <br />
        When you want to accomplish something repeatedly — say, spawning a
        new worktree and symlinking memory, node_modules, and builds —
        create scripts where possible instead of relying on AI to remember
        to do these things on its own. AI drifts over time, and simplifying
        a big complex query into "run X script when Y happens" is easier to
        manage and maintain.
      </p>
      <p>
        <strong>
          2026-05-08 — Claude REALLY likes backwards compatibility — gently
          (aggressively) push back [trust] [process]
        </strong>
        <br />
        Claude defaults to optional fields, fallback paths, and "preserve
        legacy behavior" shims — even when the codebase has no external
        consumers. Push back when you see one.
      </p>
      <p>
        <strong>
          2026-05-08 — AI wall-clock time ≠ time saved from doing it on my
          own [time-log] [calibration]
        </strong>
        <br />
        How long it took the AI to do the task is not equivalent to how
        much time it saved me from doing it on my own. It might take 30
        minutes to do the thing, but I may have only spent 5 minutes with
        it manually going through design and PR reviews. Something we
        might change in our time log.
      </p>
      <p>
        <strong>
          2026-05-08 — Leaving Claude alone with a task is kind of awesome,
          actually [autonomy] [scope-setting]
        </strong>
        <br />
        Works WAY better when seeded with as much explicit direction as
        possible. Much less so otherwise.
      ...
      <p>
        <strong>
          2026-05-08 — AI traced a thread down and back up super
          effectively, surfacing a model-honesty gap [trust] [audit]
        </strong>
        <br />
        AI helped me trace a thread down and back up super effectively,
        leading to the surfacing of a model-honesty gap.
