...
The qualitative side — observations about working with AI on this project — used to live at `AI_LEARNINGS.md` in the repo. It moved out: the in-the-moment capture ritual still happens (the PR template prompts for it), but the entries themselves are private notes kept outside the repo. The reasoning: keeping the raw observations off the public record removed the unwanted weight of "this is going on the public record" that was making in-the-moment capture feel heavy, while preserving the discipline of capturing observations at the moment they happen.

The "human substance only" rule from the original `AI_LEARNINGS.md` still applies to every PR-template AI-learnings prompt and every private-notes capture. AI assistants must not propose, draft, or synthesize observations on the author's behalf.

**At the moment of initial capture — when documenting a new AI finding into the private notes (triage/findings) — what the author says is captured verbatim. No copy-editing. No capitalization fixes. No grammar tightening. No punctuation swaps. No paragraph reflow. No filler removal. The voice IS the substance; tightening the voice strips the substance.** If the finding came in response to an AI prompt, the prompt is captured verbatim too — in a `> [Claude prompted: ...]` block above the answer — so the elicitation is part of the record. This verbatim rule applies to the initial finding capture, full stop, regardless of where the finding ultimately ends up (private notes, a future publication, a teammate's inbox, nowhere at all).

Open questions ("anything worth jotting from this session?") are fine; leading questions ("what about the X moment — did Y happen?") amount to suggesting an observation. The author drives; the AI listens.
