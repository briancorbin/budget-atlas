...
export function ExpenseBreakdown({ result }: { result: BudgetResult }) {
  ...
  return (
    <div style={{ marginBottom: 48 }}>
      <SectionTitle kicker="Where every dollar goes — Part II">
          ...
          {entries.map(([name, val], i) => {
            ...
            return (
              <div key={name} style={{
                ...
                <div style={{
                  fontSize: 11, color: T.inkMuted, marginTop: 4, fontFamily: fonts.mono,
                }}>
                  {(pct * 100).toFixed(1)}% of expenses · {(pctIncome * 100).toFixed(1)}% of take-home
                </div>
              </div>
            );
          })}
