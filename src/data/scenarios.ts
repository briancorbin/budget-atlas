import type { Scenario } from '@/types';

/**
 * Archetype scenarios spanning the income spectrum. Each is a real-feeling
 * household — the goal is to anchor exploration in concrete people rather than
 * abstract income brackets.
 *
 * `incomeB` is optional. When present, the scenario is dual-earner.
 *
 * Each scenario carries a one-line `takeaway` surfaced in the picker dropdown —
 * something the household teaches about how the system works (a marriage
 * bonus, a benefit cliff, a phase-out, the effect of a state's tax structure).
 * The takeaway should be defensible from the model itself, not editorial color.
 */
export const SCENARIOS: Scenario[] = [
  {
    id: 'min_rural',
    label: 'Minimum-wage worker · rural Mississippi',
    income: 15080,
    filing: 'single',
    city: 'rural_ms',
    kids: 0,
    lifestyle: 'modest',
    takeaway: 'Falls into the Medicaid coverage gap',
  },
  {
    id: 'min_nyc',
    label: 'Minimum-wage worker · New York City',
    income: 34320,
    filing: 'single',
    city: 'nyc',
    kids: 0,
    lifestyle: 'modest',
    takeaway: 'Same job, 2× the Mississippi minimum — rent claws most of it back',
  },
  {
    id: 'retail_phx',
    label: 'Retail worker · single mom of 2 · Phoenix',
    income: 38000,
    filing: 'head',
    city: 'phx',
    kids: 2,
    lifestyle: 'modest',
    takeaway: 'Net federal tax goes negative — refundable credits exceed withholding',
  },
  {
    id: 'admin_cmh_bbce',
    label: 'Office admin · single mom of 2 · Columbus',
    income: 46000,
    filing: 'head',
    city: 'cmh',
    kids: 2,
    lifestyle: 'moderate',
    takeaway:
      "Eligible for SNAP under Ohio's 185% BBCE rule — but the calculated benefit phases to $0",
  },
  {
    id: 'cohab_bk',
    label: 'Cohabitating creatives · 2 incomes · Brooklyn',
    income: 78000,
    incomeB: 52000,
    filing: 'single',
    city: 'nyc',
    kids: 0,
    lifestyle: 'moderate',
    takeaway: 'Asymmetric incomes — marrying would gain a federal marriage bonus',
  },
  {
    id: 'teacher_oh',
    label: 'Two teachers · 2 kids · Columbus OH',
    income: 56000,
    incomeB: 54000,
    filing: 'married',
    city: 'cmh',
    kids: 2,
    lifestyle: 'moderate',
    takeaway: 'Two near-equal incomes, MFJ — almost no marriage bonus or penalty',
  },
  {
    id: 'median_cmh',
    label: 'Median single · Columbus',
    income: 75000,
    filing: 'single',
    city: 'cmh',
    kids: 0,
    lifestyle: 'moderate',
    takeaway: 'Q3 baseline — middle-class income meets BLS Q3 spending; little discretionary left',
  },
  {
    id: 'nurse_den',
    label: 'Nurse · single · Denver',
    income: 88000,
    filing: 'single',
    city: 'den',
    kids: 0,
    lifestyle: 'moderate',
    takeaway: 'Above every safety-net threshold; flat-tax Colorado',
  },
  {
    id: 'tradesman_wy',
    label: 'Construction worker · sole earner · rural Wyoming',
    income: 62000,
    filing: 'married',
    city: 'rural_wy',
    kids: 1,
    lifestyle: 'moderate',
    takeaway: 'No state income tax + sole-earner marriage bonus',
  },
  {
    id: 'tech_aus',
    label: 'Software engineer · single · Austin',
    income: 165000,
    filing: 'single',
    city: 'aus',
    kids: 0,
    lifestyle: 'comfortable',
    takeaway: 'Texas zero-state-tax — federal + FICA carry the full bill',
  },
  {
    id: 'tech_sf',
    label: 'Software engineer · single · San Francisco',
    income: 230000,
    filing: 'single',
    city: 'sf',
    kids: 0,
    lifestyle: 'comfortable',
    takeaway: "California state tax alone tops a min-wage worker's full take-home",
  },
  {
    id: 'tech_sf_family',
    label: 'Tech couple · 2 kids · San Francisco',
    income: 230000,
    incomeB: 150000,
    filing: 'married',
    city: 'sf',
    kids: 2,
    lifestyle: 'comfortable',
    takeaway: 'Just under the $400K CTC phase-out cliff',
  },
  {
    id: 'lawyer_bos',
    label: 'Lawyer + spouse · no kids · Boston',
    income: 200000,
    incomeB: 80000,
    filing: 'married',
    city: 'bos',
    kids: 0,
    lifestyle: 'comfortable',
    takeaway: 'Asymmetric MFJ — marriage bonus vs. filing as singles',
  },
  {
    id: 'marriage_penalty_nyc',
    label: 'Two equal high earners · married · NYC',
    income: 400000,
    incomeB: 400000,
    filing: 'married',
    city: 'nyc',
    kids: 0,
    lifestyle: 'comfortable',
    takeaway: 'Marriage penalty — owes more MFJ than as two singles in the same city',
  },
  {
    id: 'family_squeeze_nyc',
    label: 'Family of 4 · $110K combined · NYC',
    income: 80000,
    incomeB: 30000,
    filing: 'married',
    city: 'nyc',
    kids: 2,
    lifestyle: 'moderate',
    takeaway: 'Q4 family squeeze — even $110K runs a monthly deficit once HCOL costs land honestly',
  },
  {
    id: 'exec_nyc',
    label: 'Finance exec + spouse · 2 kids · NYC',
    income: 500000,
    incomeB: 150000,
    filing: 'married',
    city: 'nyc',
    kids: 2,
    lifestyle: 'comfortable',
    takeaway: 'Combined federal + NY + NYC marginal rate above 50%',
  },
];
